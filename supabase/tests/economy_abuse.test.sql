...
--
-- Proves coins and card minting cannot be reached from a client. Rolled back,
...
begin
  ...
  begin
    ...
    raise exception 'FAIL: minted coins via direct insert';
  ...
  select coalesce(sum(amount),0) into v_net from public.coins_ledger where user_id = auth.uid();
  if v_net <> (select balance from public.coin_balances where user_id = auth.uid()) then
    raise exception 'FAIL: ledger nets to % but balance is %',
      v_net, (select balance from public.coin_balances where user_id = auth.uid());
  end if;
