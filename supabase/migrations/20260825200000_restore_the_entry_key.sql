-- Restores the entry charge's idempotency key, which 20260825150000 reverted.
--
-- ---------------------------------------------------------------------------
-- WHAT WENT WRONG, BECAUSE THE SHAPE OF IT WILL HAPPEN AGAIN
-- ---------------------------------------------------------------------------
--
-- `20260825150000` added `run_id` to `set_lineup` by taking the function body
-- from `20260825050000_contest_entry_fees` and editing it. That file was NOT
-- the latest definition: `20260825100000_leave_contest` had already replaced
-- the function to change one line, and rebuilding from the older file quietly
-- undid it.
--
-- The line was the entry charge's idempotency key. `leave_contest` moved it
-- from `contest_entry:<user>:<contest>` to `contest_entry:<lineup>` because the
-- first cannot tell a RETRY from a RE-ENTRY — and once a player can leave a
-- contest and come back, re-entering is a second real charge that collided
-- with the first one's key. The insert has no `on conflict`, so the symptom
-- was not a double charge but a hard failure: leaving and re-entering the same
-- contest raised `duplicate key value violates unique constraint
-- "gems_ledger_idempotency_key_idx"` and the player could not get back in.
--
-- THE STANDING RULE IN 20260824230000 EXISTS FOR EXACTLY THIS, and it says to
-- read the body back FROM THE DATABASE rather than from whichever migration
-- file looks most recent. The reason it is worth restating here is that the
-- usual way of finding the latest definition — grepping the migrations for
-- `create or replace function public.set_lineup` — MISSED IT, because a body
-- read back from Postgres comes out as `CREATE OR REPLACE FUNCTION` in upper
-- case. A case-sensitive grep for the lower-case form finds every hand-written
-- definition and none of the read-back ones, which are precisely the ones that
-- supersede them.
--
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
--
-- `20260825100000`'s body — the authoritative one — with the same three run
-- additions `20260825150000` intended, and nothing else changed:
--
--   * `hearts_at_risk` selected alongside the rest of the contest
--   * a run resolved when that figure is above zero, refusing a player whose
--     run has ended and whose carry is unclaimed
--   * `run_id` carried on the insert
--
-- Verified by diff rather than by eye: the only difference between the two
-- bodies was the key, so this is that key plus those three edits.
