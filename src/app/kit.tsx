...
const DIRECTORY_ROWS: { player: DirectoryPlayer; fixture?: string }[] = [
  {
    player: {
      ...
      seasonFp: 236.4, gamesPlayed: 16, fpPerGame: 14.8, posRank: 8, overallRank: 12, marketRank: 12, marketPosRank: 4, marketPosPool: 96, baseCoins: 51,
  ...
  },
  {
    player: {
      ...
      seasonFp: 232.6, gamesPlayed: 15, fpPerGame: 15.5, posRank: 11, overallRank: 15, marketRank: 15, marketPosRank: 5, marketPosPool: 96, baseCoins: 47,
  ...
  },
  {
    player: {
      ...
      seasonFp: 198.2, gamesPlayed: 12, fpPerGame: 16.5, posRank: 4, overallRank: 31, marketRank: 31, marketPosRank: 10, marketPosPool: 96, baseCoins: 64,
  ...
  },
  {
    player: {
      ...
      seasonFp: 288.1, gamesPlayed: 17, fpPerGame: 16.9, posRank: 5, overallRank: 2, marketRank: 2, marketPosRank: 1, marketPosPool: 96, baseCoins: 52,
  ...
  },
  {
    player: {
      ...
      seasonFp: 141.0, gamesPlayed: 16, fpPerGame: 8.8, posRank: 3, overallRank: 96, marketRank: 96, marketPosRank: 32, marketPosPool: 96, baseCoins: 38,
  ...
  },
  {
    ...
    player: {
      ...
      seasonFp: 0, gamesPlayed: 0, fpPerGame: 0, posRank: null, overallRank: null, marketRank: null, marketPosRank: null, marketPosPool: null, baseCoins: null,
        ...
        <ScrollView contentContainerStyle={styles.content}>
          ...
          >
            ...
            {/* THE FINISHED STATE, WHICH IS THE THIRD BAND IN THE PAST TENSE.
                ...
                what it paid is what its cards earned — 148 coins, which is the
            ...
            <KitEntered
              ...
              settled={{ result: null, coins: null }}
              lock={{ at: DEMO_LOCK_PAST, locked: true, now: DEMO_NOW }}
            />
          ...
          <Section
            ...
            {/* THE SAME ROWS WITH A PLACE ON THEM. Only the Top board passes a
                rank — see `PlayerRow.rank` — and the two panels sit together
                here because the difference between them is the whole argument:
                a number beside a name is honest when the order is the subject
                and a lie when a sort chip can change it. */}
            <Panel>
              {DIRECTORY_ROWS.map((r, i) => (
                <PlayerRow
                  ...
                  onPress={() => {}}
                  fixture={r.fixture}
                  /* ONE NUMBER AND ITS UNIT, as every board draws it now. The
                     figure column used to stack a measure over a coin line and
                     no longer can — see `RowFigure`. Rows with no games show
                     the quiet dash, which is the state the whole board is in
                     before week one. */
                  figure={{
          ...
          <Section
            ...
            note="The coin figure is how selling is discovered from the inventory.">
            <Panel>
          ...
          <Section
            ...
            <ConfirmDialog
              ...
              body="Christian McCaffrey · 2026 card. You will receive 150 coins. The copy and everything it has earned — 1,285 FP over 41 starts — are gone for good, and buying the player again starts a new card at bronze."
          ...
          <Section
            ...
            <Modal
              ...
              <KitPull
                ...
                onSell={(id) => {
                  setPullDisposed((held) => new Map(held).set(id, { kind: 'sold', coins: 8 }));
                  spendPullCard(id);
                }}
                onCommit={(id, code) => {
...
function KitPull({
  ...
  let earned = 0;
  for (const d of disposed.values()) earned += d.coins;
