/**
 * The scoreboard, as a tab of its own.
 *
 ...
 *
 * IT WAS A BAND ACROSS THE TOP OF THE LINEUP, and before that it was a page
 * under it. The band was the right answer while the bottom bar belonged to the
 * fantasy game and every one of its five tabs had to earn its slot: the two
 * facts that decide a lineup are who is playing and how it is going, so they
 * were put next to the decision rather than a navigation away from it.
 *
 * The bar belongs to the whole app now, which changes the arithmetic. The
 * league's week is not a fantasy board — it is the same fact for everybody
 * playing — so it sits beside Fantasy rather than inside it, and having a page
 * again buys back the two things the band could not hold at 60pt tall: a week
 * picker that reaches back through the season, and the per-game leaders that
 * turn a fixture list into a reason to open a pack.
 *
...
export default function ScoresScreen() {
  ...
  const [current, setCurrent] = useState<Slate | null>(null);
  const [seeded, setSeeded] = useState(false);

  /**
   * The week the reader ASKED for, which is a different thing from the week
   * they are looking at. Null until they touch the picker.
   */
  const [picked, setPicked] = useState<Slate | null>(null);

  /**
   * One instant, taken at mount, for "has this week kicked off yet".
   *
   * Reading the clock during render is impure — the same render would answer
   * differently either side of a kickoff — and it buys nothing here: this only
   * decides which week the picker OPENS on, before the RPC has landed, and a
   * scoreboard that silently jumped to the next week while you were reading
   * the last one would be a bug rather than a feature.
   */
  ...
  useEffect(() => {
    void (async () => {
      ...
      if (!error && row) {
        ...
        setCurrent({ season: row.season, seasonType: row.season_type, week: row.week });
      }
      setSeeded(true);
    })();
  }, []);
  ...
  /**
   * If the RPC gave us nothing, open on the last slate that has actually kicked
   * off — a scoreboard opening on week 1 in December is useless.
   *
   * DERIVED, not stored. This used to be an effect that wrote the fallback into
   * the same state the RPC and the picker wrote to, which made one variable
   * mean three different things and left the screen rendering once with no week
   * at all before the effect caught up. Three sources, three variables, and one
   * expression below that says which of them wins.
   */
  const fallback = useMemo<Slate | null>(() => {
      ...
      .find((s) =>
        games.some(
          (g) =>
            ...
            g.week === s.week &&
            g.startsAt !== null &&
            new Date(g.startsAt).getTime() <= mountedAt,
        ),
      );
    return started ?? slates[0];
  }, [slates, games, mountedAt]);

  /* A deliberate choice beats the competition's own idea of now, which beats a
     guess from the fixture list. */
  const slate = picked ?? current ?? fallback;
